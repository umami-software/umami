...
export async function POST(
  ...
  const { paymentProviderId } = await params;
  const existing = await getPaymentProviderById(paymentProviderId);
  ...
  const row = await updatePaymentProvider(paymentProviderId, update);
  const rawKey = decrypt(row.apiKey, secret());
...
export async function DELETE(
  ...
  await deletePaymentProviderById(paymentProviderId);
