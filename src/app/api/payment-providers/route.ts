...
import {
  getPaymentProvidersPage,
  maskKey,
  updatePaymentProviderWebhook,
  upsertPaymentProviderForTeam,
  upsertPaymentProviderForUser,
} from '@/queries/prisma';
...
export async function GET(request: Request) {
  ...
  const result = await getPaymentProvidersPage(where, filters);
...
export async function POST(request: Request) {
  ...
  const row = userId
    ? await upsertPaymentProviderForUser(
        userId,
        provider,
        name,
        encryptedKey,
    ...
    : await upsertPaymentProviderForTeam(
        teamId,
        provider,
        name,
        encryptedKey,
  ...
  if (!webhookId && !webhookSecret && process.env.APP_URL) {
    ...
    const webhook = await stripe.webhookEndpoints.create({
      url: `${process.env.APP_URL}/api/payment-providers/${row.id}/webhook`,
      enabled_events: [
        'invoice.created',
        'invoice.finalized',
        'invoice.paid',
    ...
    await updatePaymentProviderWebhook(row.id, {
