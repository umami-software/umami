...
const FUNCTION_NAME = 'getMRRRelational';

// Postgres implementation of getMRR, kept as the reference for the numbers a
// ClickHouse implementation must match. payment_provider_invoice only exists in Postgres,
// so this bypasses the ClickHouse/Prisma runQuery dispatch used by analytics
// queries that can live in either backend.
export async function getMRRRelational(
  ...
  const rows = await rawQuery(
    ...
    licensed_by_month AS (
      ...
      FROM payment_provider_invoice bil
      ...
      WHERE bil.invoice_status = 'paid'
        ...
        AND bil.payment_provider_id = {{paymentProviderId}}::uuid
    ...
    -- always has a row per month even when there is no payment activity.
    months AS (
