...
describe('getWebsiteEvents', () => {
  test('postgres counts from the lightweight filtered event query and enriches only the paged rows', async () => {
    ...
    expect(dataQuery).toContain('where event_data.website_event_id = website_event.event_id');
    expect(dataQuery).toContain('and event_data.website_id = {{websiteId::uuid}}');
    ...
    expect(dataQuery).not.toContain('created_at between {{startDate}} and {{endDate}}) AS "hasData"');
  ...
  test('clickhouse counts from the lightweight filtered event query and resolves hasData from paged ids', async () => {
    ...
    expect(dataQuery).toContain('and event_id in (select event_id from paged_events)');
    expect(dataQuery).toContain('order by paged_events.created_at desc');
    expect(dataQuery).not.toContain('event_id IN (select event_id');
    expect(result).toEqual({
      data: [{ id: 'event-1' }],
      count: '25',
      page: 1,
